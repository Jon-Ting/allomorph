...
## EAM database tool
Python version (create_eam.py) written by Germain Clavier germain.clavier@gmail.com
...
### Warning
Please note that the parameter sets used here are all optimized
...
## Usage
create_eam.py [-h] [-n NAME [NAME ...]] [-nr NR] [-nrho NRHO]
...
3) this will create an .eam.alloy potential file (modified by Jon-Ting to be placed in setfl/ and end with .set instead)

* in DYNAMO or LAMMPS context, the created file is referred to as a setfl file that can be used with the LAMMPS pair_style eam/alloy command.
