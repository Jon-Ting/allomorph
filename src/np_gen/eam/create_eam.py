...
def prof(at, r):
    atom = Database[at]
    f = np.zeros(r.shape)
    # Avoid division by zero and handle physical cutoffs
    mask = r >= 0.5
    f[mask] = atom.fe * np.exp(-atom.beta1 * (r[mask] / atom.re - 1.0))
...
def create_eam(argv=None):
    ...
    parser.add_argument("-n", "--names", dest="name", nargs="+", required=True,
                        help="Element names (e.g., Au Pt Pd)")
    parser.add_argument("-nr", dest="nr", type=int, default=2000,
                        help="Number of points in r space (must be > 1, default: 2000)")
    ...
    if args.nr <= 1:
        sys.exit("Error: -nr must be greater than 1.")
    if args.nrho <= 1:
        sys.exit("Error: -nrho must be greater than 1.")
    ...
    for n in atnames:
        if n not in Database:
            output = f"Element {n} not found in database.\n"
            valid = f"Supported elements are: {' '.join(Database.keys())}"
            sys.exit(output + valid)

    ntypes = len(atnames)
    outfilename = Path(args.output_dir) / ("".join([*atnames, ".set"]))
    rhor = {}
    Fr = {}
    ...
    write_file(atnames, str(outfilename), Fr, rhor, z2r, nrho, drho, nr, dr, rc)
    logger.info(f"Successfully generated {outfilename}")
